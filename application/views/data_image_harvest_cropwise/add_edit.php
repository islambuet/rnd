...
?>
<form class="form_valid" id="save_form" action="<?php echo base_url();?>data_image_harvest_cropwise/index/save" method="post">
    <div class="row widget">
        ...
        <div class="row show-grid" style="display: none;"  id="harvest_no_container">
            <div class="col-xs-4">
                <label class="control-label pull-right"><?php echo $this->lang->line('LABEL_HARVEST_NO');?><span style="color:#FF0000">*</span></label>
            </div>
            <div class="col-xs-4">
    ...
    <div class="row widget" id="harvest_cropwise_images">
...
//        $(".form_valid").validationEngine();
        ...
        $(document).on("change", "#crop_type_id", function(event)
        {
            ...
            {
                $.ajax({
                    url: base_url+"data_image_harvest_cropwise/get_harvest_no_for_data_image",
                    type: 'POST',
                    dataType: "JSON",
        ...
        $(document).on("change", "#harvest_no", function(event)
        {
            ...
            {
                $.ajax({
                    url: base_url+"data_image_harvest_cropwise/index/list",
                    type: 'POST',
                    dataType: "JSON",
                    data:{year:$("#year").val(),season_id:$("#season_id").val(),crop_id:$("#crop_id").val(),crop_type_id:$("#crop_type_id").val(),harvest_no:$("#harvest_no").val()},
                    success: function (data, status)
                    {
