...
?>
<form class="form_valid" id="save_form" action="<?php echo base_url();?>hofc_principal_data_entry/index/save" method="post">
    <div class="row widget">
        ...
        <div class="row show-grid">
            <div class="col-xs-4">
                <label class="control-label pull-right"><?php echo $this->lang->line('LABEL_SELECT_PRINCIPAL');?><span style="color:#FF0000">*</span></label>
            </div>
            <div class="col-xs-4">
...
//        $(".form_valid").validationEngine();
        ...
        $(document).on("change", "#crop_type_id", function(event)
        {
            ...
            {
                $.ajax({
                    url: base_url+"hofc_principal_data_entry/get_varieties",
                    type: 'POST',
                    dataType: "JSON",
                    data:{year:$("#year").val(),season_id:$("#season_id").val(),crop_id:$("#crop_id").val(),crop_type_id:$("#crop_type_id").val(),principal_id:$('#principal_id').val()},
                    success: function (data, status)
                    {
        ...
        $(document).on("change", "#variety_id", function(event)
        {
            ...
            {
                $.ajax({
                    url: base_url+"hofc_principal_data_entry/index/list",
                    type: 'POST',
                    dataType: "JSON",
                    data:{year:$("#year").val(),season_id:$("#season_id").val(),crop_id:$("#crop_id").val(),crop_type_id:$("#crop_type_id").val(),variety_id:$('#variety_id').val(),principal_id:$('#principal_id').val()},
                    success: function (data, status)
                    {
