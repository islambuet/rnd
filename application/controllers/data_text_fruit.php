...
class Data_text_fruit extends ROOT_Controller
{
    private  $message;
    ...
    {
        $data['title']="Fruit Text Report";

        $data['crops'] = System_helper::get_ordered_crops();
        $data['seasons'] = Query_helper::get_info('rnd_season', '*', array());
        $ajax['status']=true;
        $ajax['content'][]=array("id"=>"#content","html"=>$this->load->view("data_text_fruit/add_edit",$data,true));
    ...
    public function rnd_list()
    {
        ...
        else
        {
            ...
            $ajax['status']=true;
            $ajax['content'][]=array("id"=>"#fruit_text","html"=>$this->load->view("data_text_fruit/list",$data,true));
            $this->jsonReturn($ajax);
        }
    }
    ...
    public function rnd_save()
    {
        ...
        else
        {
            ...
            if($id>0)
            {
                ...
                Query_helper::update('rnd_data_text_fruit',$data,array('id = '.$id));
            }
            else
            {
                ...
                Query_helper::add('rnd_data_text_fruit',$data);
            }
    ...
    public function get_varieties_for_data_text()
    {
        ...
        $crop_id = $this->input->post('crop_id');
        $crop_type_id = $this->input->post('crop_type_id');
        $data['varieties']=$this->data_text_fruit_model->get_varieties($year,$season_id,$crop_id,$crop_type_id);
...
//            {
//                $valid=false;
//                $this->message.=$this->lang->line('PLEASE_COMPLETE_FRUIT_SETUP').'<br>';
//
//            }
