...
class Setup_image_flowering extends ROOT_Controller
{
    private  $message;
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->load->model("setup_image_flowering_model");
    }
    ...
    public function rnd_add_edit()
    {
        ...
        $ajax['status']=true;
        $ajax['content'][]=array("id"=>"#content","html"=>$this->load->view("setup_image_flowering/add_edit",$data,true));
        ...
        $ajax['page_url']=base_url()."setup_image_flowering/index/add_edit";
    ...
    public function rnd_list()
    {
        ...
        else
        {
            ...
            if($config)
            {
                ...
                foreach($this->config->item("flowering_image") as $val=>$flowering)
                {
                    if($images[$val])
                    {
            ...
            {
                $old_config = Query_helper::get_info("rnd_setup_image_flowering","*",array('year = '.($year-1),'season_id = '.$season_id,'crop_id = '.$crop_id,'crop_type_id = '.$crop_type_id),1);
