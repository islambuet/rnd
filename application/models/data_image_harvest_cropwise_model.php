...
class Data_image_harvest_cropwise_model extends CI_Model
{
    ...
    public function get_varieties($year,$season_id,$crop_id,$crop_type_id,$day_number=15)
    {

        $sub_query='(SELECT * FROM rnd_data_image_fifteen_days where year="'.$year.'" AND season_id='.$season_id.' AND crop_id='.$crop_id.' AND crop_type_id='.$crop_type_id.' AND day_number='.$day_number.')';
