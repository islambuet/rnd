...
class Data_image_harvest_cropwise_model extends CI_Model
{

    public function __construct() {
        parent::__construct();
    }
    public function get_varieties($year,$season_id,$crop_id,$crop_type_id,$harvest_no)
    {

        $sub_query='(SELECT * FROM rnd_data_image_harvest_cropwise where year="'.$year.'" AND season_id='.$season_id.' AND crop_id='.$crop_id.' AND crop_type_id='.$crop_type_id.' AND harvest_no='.$harvest_no.')';

        $this->db->from('rnd_variety rv');
        $this->db->select('rv.*');
        $this->db->select('rdihc.images images,rdihc.remarks,rdihc.id rdihc_id');
        ...
        $this->db->join($sub_query.' rdihc', 'rdihc.variety_id = rvs.variety_id', 'left');
