...
class Data_text_harvest_cropwise_model extends CI_Model
{
    ...
    public function get_variety_info($year,$season_id,$crop_id,$crop_type_id,$variety_id,$harvest_number)
    {
        ...
        $text_sub_query='(SELECT id,info,variety_id FROM rnd_data_text_harvest WHERE year="'.$year.'" AND season_id ='.$season_id.' AND crop_id ='.$crop_id.' AND crop_type_id ='.$crop_type_id.' AND variety_id ='.$variety_id.' AND harvest_number ='.$harvest_number.')';
        $this->db->from('rnd_variety rv');
        $this->db->select('rv.*');
