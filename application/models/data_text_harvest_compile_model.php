...
class Data_text_harvest_compile_model extends CI_Model
{
    ...
    public function get_variety_info($year,$season_id,$crop_id,$crop_type_id,$variety_id)
    {
        ...
        $text_sub_query='(SELECT id,info,ranking,variety_id FROM rnd_data_text_harvest_compile WHERE year="'.$year.'" AND season_id ='.$season_id.' AND crop_id ='.$crop_id.' AND crop_type_id ='.$crop_type_id.' AND variety_id ='.$variety_id.')';
        $this->db->from('rnd_variety rv');
        $this->db->select('rv.*');
        $this->db->select('dass.sowing_date,dass.transplanting_date');
        $this->db->select('rdtfd.info,rdtfd.id data_text_id,rdtfd.ranking');
