...
class Data_text_fruit_model extends CI_Model
{
    ...
    public function get_variety_info($year,$season_id,$crop_id,$crop_type_id,$variety_id)
    {
        ...
        $text_sub_query='(SELECT id,info,ranking,variety_id FROM rnd_data_text_fruit WHERE year="'.$year.'" AND season_id ='.$season_id.' AND crop_id ='.$crop_id.' AND crop_type_id ='.$crop_type_id.' AND variety_id ='.$variety_id.')';
